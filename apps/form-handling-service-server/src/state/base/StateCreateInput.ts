/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { FormCreateNestedManyWithoutStatesInput } from "./FormCreateNestedManyWithoutStatesInput";
import {
  ValidateNested,
  IsOptional,
  IsString,
  MaxLength,
} from "class-validator";
import { Type } from "class-transformer";
import { OrganizationCreateNestedManyWithoutStatesInput } from "./OrganizationCreateNestedManyWithoutStatesInput";

@InputType()
class StateCreateInput {
  @ApiProperty({
    required: false,
    type: () => FormCreateNestedManyWithoutStatesInput,
  })
  @ValidateNested()
  @Type(() => FormCreateNestedManyWithoutStatesInput)
  @IsOptional()
  @Field(() => FormCreateNestedManyWithoutStatesInput, {
    nullable: true,
  })
  forms?: FormCreateNestedManyWithoutStatesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => OrganizationCreateNestedManyWithoutStatesInput,
  })
  @ValidateNested()
  @Type(() => OrganizationCreateNestedManyWithoutStatesInput)
  @IsOptional()
  @Field(() => OrganizationCreateNestedManyWithoutStatesInput, {
    nullable: true,
  })
  organizations?: OrganizationCreateNestedManyWithoutStatesInput;
}

export { StateCreateInput as StateCreateInput };
